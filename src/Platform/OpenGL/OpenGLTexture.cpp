...
namespace RayMagic {
		...
		: m_Path(path)
	{
		...
		RM_CORE_ASSERT(data, "Failed to load image!");
		m_Width = width;
		m_Height = height;
		...
		RM_CORE_ASSERT(internalFormat && dataFormat, "Format not supported!");
	...
	void OpenGLTexture2D::SetData(void* data, uint32_t size)
	{
		...
		RM_CORE_ASSERT(size == m_Width * m_Height * bpc, "Data must be entire texture!");
